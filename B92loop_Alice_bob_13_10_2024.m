...
for run = 1:num_runs
    ...
    for j = 1:length(N_values)
        ...
        % Step 2: Generate what Bob sees based on the probabilities provided
        bob_results = strings(1, N);
        for i = 1:N
            if alice_bits(i) == 0  % Alice sends 0 in basis +
                outcome = randsample(["0+", "1x", "0x"], 1, true, [0.5, 0.25, 0.25]);
            else  % Alice sends 1 in basis x
                outcome = randsample(["1x", "1+", "0+"], 1, true, [0.5, 0.25, 0.25]);
            end
...
title('Proportion of "Granted" Bits as a Function of N with Error Region', 'FontName', 'Times New Roman', 'FontSize', 20);
grid on;
